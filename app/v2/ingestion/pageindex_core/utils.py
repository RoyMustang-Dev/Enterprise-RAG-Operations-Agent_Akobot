...
def ChatGPT_API(model, prompt, api_key=GROQ_API_KEY, chat_history=None):
    max_retries = 10
    client = openai.OpenAI(base_url="https://api.groq.com/openai/v1", api_key=api_key)
    for i in range(max_retries):
        try:
...
async def ChatGPT_API_async(model, prompt, api_key=GROQ_API_KEY):
    ...
    for i in range(max_retries):
        try:
            async with openai.AsyncOpenAI(base_url="https://api.groq.com/openai/v1", api_key=api_key) as client:
...
def extract_text_from_pdf(pdf_path):
    pdf_reader = _FitzPDFReader(pdf_path)
