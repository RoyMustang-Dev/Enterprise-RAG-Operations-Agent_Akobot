...
# Removed app.infra.database imports for V2 memory-only extraction
...
class CrawlerService:
    ...
    async def _http_only_crawl(self, seed_urls: list, session_id: str, rp, max_depth: int, max_urls: int, max_seconds: float) -> dict:
        ...
        while not queue.empty():
            ...
            self.results_memory.append({"session_id": session_id, "url": current_url, "title": title, "content": clean_content, "depth": depth, "status": "success"})
            pages_crawled += 1
    ...
    async def _memory_writer(self, queue, on_batch_extracted=None):
        """
        ...
        while True:
            ...
            self.results_memory.append({"session_id": item[0], "url": item[1], "title": item[2], "content": item[3], "depth": item[4], "status": item[5]})
    ...
    async def crawl_url(self, url: str, save_folder: str = None, simulate: bool = False, recursive: bool = False, max_depth: int = 1, stop_event: asyncio.Event = None, on_batch_extracted=None) -> dict:
        ...
        # In V2, we don't strictly deduplicate across runs using DB.
        ...
        async with async_playwright() as p:
            ...
            # Start DB writer
            db_task = asyncio.create_task(self._memory_writer(db_queue, on_batch_extracted))
            ...
            rows = [r for r in self.results_memory if r['session_id'] == session_id and r['status'] == 'success']
